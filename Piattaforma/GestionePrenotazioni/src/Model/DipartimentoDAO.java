...
public class DipartimentoDAO {
	...
	public synchronized int doRetrieveByName(String nome) {
		try (Connection con = DriverManagerConnectionPool.getConnection()) {
			PreparedStatement ps = con.prepareStatement("SELECT Dipartimento.Nome, Dipartimento.AmmDip FROM Dipartimento WHERE Nome=?;");
			ps.setString(1, nome);
