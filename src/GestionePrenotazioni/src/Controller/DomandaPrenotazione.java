...
public class DomandaPrenotazione extends ServletBasic {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		int temp = prenDAO.controlloOra(data, Integer.parseInt(oraInizio), Integer.parseInt(oraFine), aula, edificio);
		...
		if (temp == 1) {
			...
			request.setAttribute("messaggio", "Richiesta non avvenuta orario  gia prenotato!");
			RequestDispatcher view = request.getRequestDispatcher("NavAula?aula=" + aula);
			view.forward(request, response);
		}
